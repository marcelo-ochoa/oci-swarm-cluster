...
resource "oci_core_virtual_network" "oci_swarm_main_vcn" {
  cidr_block     = lookup(var.network_cidrs, "MAIN-VCN-CIDR")
  compartment_id = var.compartment_ocid
  display_name   = "oci-swarm-main-${random_string.deploy_id.result}"
  dns_label      = "swarmmain${random_string.deploy_id.result}"
  freeform_tags  = local.common_tags
}

resource "oci_core_virtual_network" "oci_swarm_lb_vcn" {
  ...
  dns_label      = "swarmlb${random_string.deploy_id.result}"
  freeform_tags  = local.common_tags

  count = var.create_secondary_vcn ? 1 : 0
}

resource "oci_core_subnet" "oci_swarm_main_subnet" {
  cidr_block                 = lookup(var.network_cidrs, "MAIN-SUBNET-REGIONAL-CIDR")
  display_name               = "oci-swarm-main-${random_string.deploy_id.result}"
  dns_label                  = "swarmmain${random_string.deploy_id.result}"
  security_list_ids          = [oci_core_security_list.oci_swarm_security_list.id]
  compartment_id             = var.compartment_ocid
  vcn_id                     = oci_core_virtual_network.oci_swarm_main_vcn.id
  route_table_id             = oci_core_route_table.oci_swarm_main_route_table.id
  dhcp_options_id            = oci_core_virtual_network.oci_swarm_main_vcn.default_dhcp_options_id
  prohibit_public_ip_on_vnic = (var.instance_visibility == "Private") ? true : false
  freeform_tags              = local.common_tags
}

resource "oci_core_subnet" "oci_swarm_lb_subnet" {
  cidr_block                 = lookup(var.network_cidrs, (var.create_secondary_vcn ? "LB-SUBNET-REGIONAL-CIDR" : "MAIN-LB-SUBNET-REGIONAL-CIDR"))
  display_name               = "oci-swarm-lb-${random_string.deploy_id.result}"
  dns_label                  = "swarmlb${random_string.deploy_id.result}"
  security_list_ids          = [oci_core_security_list.oci_swarm_lb_security_list.id]
  compartment_id             = (var.lb_compartment_ocid != "") ? var.lb_compartment_ocid : var.compartment_ocid
  ...
  route_table_id             = oci_core_route_table.oci_swarm_lb_route_table.id
  dhcp_options_id            = var.create_secondary_vcn ? oci_core_virtual_network.oci_swarm_lb_vcn[0].default_dhcp_options_id : oci_core_virtual_network.oci_swarm_main_vcn.default_dhcp_options_id
  prohibit_public_ip_on_vnic = false
  freeform_tags              = local.common_tags
}

resource "oci_core_route_table" "oci_swarm_main_route_table" {
  compartment_id = var.compartment_ocid
  vcn_id         = oci_core_virtual_network.oci_swarm_main_vcn.id
  display_name   = "oci-swarm-main-${random_string.deploy_id.result}"
  freeform_tags  = local.common_tags
  ...
  dynamic "route_rules" {
    ...
    content {
      ...
      network_entity_id = oci_core_service_gateway.oci_swarm_service_gateway.id
    }
  }
  ...
  dynamic "route_rules" {
    ...
    content {
      ...
      network_entity_id = oci_core_internet_gateway.oci_swarm_internet_gateway.id
    }
  }
...
resource "oci_core_route_table" "oci_swarm_lb_route_table" {
  compartment_id = (var.lb_compartment_ocid != "") ? var.lb_compartment_ocid : var.compartment_ocid
  vcn_id         = var.create_secondary_vcn ? oci_core_virtual_network.oci_swarm_lb_vcn[0].id : oci_core_virtual_network.oci_swarm_main_vcn.id
  display_name   = "oci-swarm-lb-${random_string.deploy_id.result}"
  freeform_tags  = local.common_tags
...
resource "oci_core_nat_gateway" "oci_swarm_nat_gateway" {
  block_traffic  = "false"
  compartment_id = var.compartment_ocid
  display_name   = "oci-swarm-nat-gateway-${random_string.deploy_id.result}"
  vcn_id         = oci_core_virtual_network.oci_swarm_main_vcn.id
  freeform_tags  = local.common_tags
...
resource "oci_core_internet_gateway" "oci_swarm_internet_gateway" {
  compartment_id = (var.lb_compartment_ocid != "") ? var.lb_compartment_ocid : var.compartment_ocid
  display_name   = "oci-swarm-internet-gateway-${random_string.deploy_id.result}"
  vcn_id         = var.create_secondary_vcn ? oci_core_virtual_network.oci_swarm_lb_vcn[0].id : oci_core_virtual_network.oci_swarm_main_vcn.id
  freeform_tags  = local.common_tags
}

resource "oci_core_service_gateway" "oci_swarm_service_gateway" {
  compartment_id = var.compartment_ocid
  display_name   = "oci-swarm-service-gateway-${random_string.deploy_id.result}"
  vcn_id         = oci_core_virtual_network.oci_swarm_main_vcn.id
  services {
...
resource "oci_core_local_peering_gateway" "lb_local_peering_gateway" {
  compartment_id = (var.lb_compartment_ocid != "") ? var.lb_compartment_ocid : var.compartment_ocid
  vcn_id         = oci_core_virtual_network.oci_swarm_lb_vcn[0].id
